...
使用 Agent 的图片识别能力，按以下区域**逐区扫描**提取所有可见数据：

**① 顶部/标题区域**（通常包含）：
- 股票代码、股票名称
- 当前价格、涨跌额、涨跌幅
- 成交量、成交额、换手率、市盈率等摘要数据

**② 主图区域的文字标注**（通常在图表左上方或图例处）：
- 均线数值标注，如 `MA5:98.32  MA10:96.15  MA20:93.40`
- 布林带数值标注，如 `BOLL:95.2  UB:101.3  LB:89.1`
- 当前 K 线的开高低收数值（如果有标注）

**③ 副图区域的文字标注**（MACD/KDJ/RSI 等指标子图）：
- MACD 数值标注，如 `MACD:1.23  DIF:2.45  DEA:1.22` 或 `DIFF:  DEA:  MACD:`
- KDJ 数值标注，如 `K:75.3  D:68.1  J:89.7`
- RSI 数值标注，如 `RSI6:62.4  RSI12:58.7`
- 成交量数值标注（如 `VOL:123456` 或 `量:12.3万`）

**④ 图形形态观察**（文字标注不足时的补充）：
- K 线形态：最近几根 K 线是阳线/阴线、大小、是否有长上影/下影
- 均线排列方向：多头排列（短期在上）/ 空头排列 / 交织
- MACD 柱状图：红柱/绿柱、柱体是在放大还是缩小
- 成交量柱状：近期是放量还是缩量

**⚠️ 关键**：优先提取**图表上标注的具体数值**（MA、KDJ、MACD 的数字）。这些数值通常以小字显示在图表区域的顶部或左上角，不要忽略。只有当图表没有文字标注时，才通过观察图形形态进行定性描述。
